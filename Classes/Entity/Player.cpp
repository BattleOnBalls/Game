...
bool Player::init(Vec2 position, Node *map)
{
	...
	player_name = "HelloWorld";     //系统默认用户名
	_map = map;
	skin_id = 1;
	_state = State::NORMAL;
	combine_bool = true;
...
void  Player::dividePlayer()
{
	...
	for (auto division = division_list.begin(); division != division_list.end(); division++)
	{
		...
		if (score >= PLAYER_MIN_DIVIDE_SCORE)
		{
			...
			Vec2 newPosition = Vec2((2 * radius + PLAYER_MIN_DIVIDE_DISTANCE) * cosf(angle),
				(2 * radius + PLAYER_MIN_DIVIDE_DISTANCE) * sinf(angle));
	...
	for (auto division : division_list)
	{
		if (division != NULL)
		{
			...
			if (score >= PLAYER_MIN_SPIT_SCORE)
			{
				...
				sporeMap.insert(globalID, spore);//map和sporeMap有什么玄机？
				globalID++;
			}
		}
	}
	...
	for (auto division : division_list)
	{
		if (division != NULL)
		{
			...
			if (division->collidePrick(prick))
			{
				...
				{
					int num = PLAYER_MAX_DIVISION_NUM - division_num;
					int splitScore = prickScore + divisionScore - num * SPORE_SCORE;
					division->eatPrick(splitScore);
					...
					for (int i = 0; i < num; i++)
					{
						...
						auto splitDivision = this->createDivision(division->getPosition(), velocity, SPORE_SCORE);
						_map->addChild(splitDivision, SPORE_SCORE);  //按分值决定割裂出的小球的渲染顺序
...
int Player::collideDivision(PlayerDivision *pdivision)
{
	...
	for (auto division : division_list)
	{
		...
		if (distance < abs(playerRadius - rivalRadius))  //abs:取绝对值
		{
