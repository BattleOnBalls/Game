...
public:
	...
	void eatPrick(int score);     //在有效撞刺后，将_score设置成score

	void eatRival(int score);     //吃敌人加分

	void spitSpore();     //吐孢子减分
	void divide();     //将球球的分值减半并且calculateData

	virtual void setScore(int score);     //设置分值
	void setVelocity(Vec2 v);     //
	void setPrePosition(const Vec2 &position);     //
	void setPlayerName(const std::string &playername);     //设置用户名
